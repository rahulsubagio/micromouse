...
void loop() {
  ...
  //leftError, rightError, frontError = 
  walls();
...
void getSpeed(int nextDir) { //, int leftError, int rightError, int frontError) {     // motor will not stop except in a STOP direction
  ...
  forward = constrain(forward, 30, 150); 
  ...
  if (nextDir ==  LEFT) {
    ...
    leftMotor = constrain(leftMotor, 0, 60);      // limitation. minimum value is 0 and maximum value is 30. 
    ...
    rightMotor = constrain(rightMotor, 0, 60);
...
int getDirection() {
  ...
  } else if ( leftSensor <= 5 || leftSensor > 200 && rightSensor <= 5 || rightSensor > 200 && frontSensor <= 5 || frontSensor > 200 ) {
    DIRECTION = STOP;//0
    
  } else if (frontwall = true) {
